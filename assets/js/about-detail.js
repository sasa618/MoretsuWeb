...
2004年、新潟県三条市生まれ。幼少期の小児がん（腎明細胞肉腫）の闘病経験から医療の道を志すも、医療系スタートアップとの出会いを機に「仕組みによる社会インパクト」の創出を追求する。18歳から営業代行や事業開発、組織立ち上げを経験し、複数のベンチャー企業で現場と戦略の両面から経営を推進。2025年10月、株式会社モーレツを創業し取締役COOに就任。現在は組織とテクノロジーを軸とした実行型パートナーとして、企業の非連続的な成長を加速させている。開志専門職大学在学中。`,
  ...
  function renderRaw(raw) {
    const tokens = raw.match(/\*\*\*[\s\S]*?\*\*\*|\*\*[\s\S]*?\*\*|\*[\s\S]*?\*/g) || [];
    let cursor = 0;
    let html = '';

    tokens.forEach((token) => {
      const index = raw.indexOf(token, cursor);
      const plain = raw.slice(cursor, index).trim();
      if (plain) {
        plain
          .split(/\n\s*\n/)
          .map((part) => part.trim())
          .filter(Boolean)
          .forEach((part) => {
            html += `<p class="about-detail__text">${withBreaks(part)}</p>`;
          });
      }

      if (token.startsWith('***')) {
        const content = token.slice(3, -3).trim();
        if (content) html += `<p class="about-detail__profile-title">${withBreaks(content)}</p>`;
      } else if (token.startsWith('**')) {
        const content = token.slice(2, -2).trim();
        if (content) html += `<h2 class="about-detail__section-title">${withBreaks(content)}</h2>`;
      } else {
        const content = token.slice(1, -1).trim();
        if (content) html += `<p class="about-detail__accent">${withBreaks(content)}</p>`;
      }
