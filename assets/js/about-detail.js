    ...
    return raw
      ...
      .map((block) => {
        ...
        if (block.startsWith('**') && block.endsWith('**')) {
          ...
          return `<h2 class="about-detail__block about-detail__block--double">${renderInline(text)}</h2>`;
        }

        if (block.startsWith('*') && block.endsWith('*')) {
          const text = block.slice(1, -1).trim();
          return `<p class="about-detail__block about-detail__block--single">${renderInline(text)}</p>`;
        }

        return `<p class="about-detail__block about-detail__block--body">${renderInline(block)}</p>`;
      })
      .join('');
  }
  ...
  root.innerHTML = `
    <div class="about-detail__inner">
      ...
      <div class="about-detail__hero">
        <img class="about-detail__image" src="${leader.image}" alt="${leader.imageAlt}" />
        <div class="about-detail__head">
          <p class="about-eyebrow">Leadership Message</p>
          <p class="about-detail__role">${leader.role}</p>
          <h1 class="about-detail__name">${leader.name}</h1>
        </div>
      </div>
